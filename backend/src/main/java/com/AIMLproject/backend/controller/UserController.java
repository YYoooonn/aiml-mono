...
public class UserController {
	...
	public ResponseEntity<?> register(@RequestBody RegisterReq req) {
		try {
			String encodedPassword = userService.saveUser(req.getUsername(), req.getPassword());
			return ResponseEntity.status(HttpStatus.CREATED).body(new RegisterRes(encodedPassword));
		} catch (Exception e) {
